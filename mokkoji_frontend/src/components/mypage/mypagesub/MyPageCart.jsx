...
import allProductData from '../../product/ProductObject'

function MyPageCart({ change, setChange }) {

    const currentUser = JSON.parse(sessionStorage.getItem("LoginUserInfo"));
    // const allProductData = JSON.parse(sessionStorage.getItem("goodsList"));
    ...
    let cartGoods = user.mypage.basket.map((userBasket) => {
        let findItem = allProductData.find((item) =>
            item.id === userBasket.productId
        );     // findItem
        ...
        let calculateTotalPrice = () => {
            ...
            const packagingStartIndex = userBasket.options.packagingSelect.indexOf('(+');
            const packagingEndIndex = userBasket.options.packagingSelect.indexOf('원)');
            const contentSelectStartIndex = userBasket.options.contentSelect.indexOf('(+');
            const contentSelectEndIndex = userBasket.options.contentSelect.indexOf('원)');

            // 포장 옵션에 따라 추가 금액 설정
            if (packagingStartIndex !== -1 && packagingEndIndex !== -1) {
                packagingPrice = +(userBasket.options.packagingSelect.slice(packagingStartIndex + 2, packagingEndIndex))
            }

            // 내용 옵션에 따라 추가 금액 설정
            if (contentSelectStartIndex !== -1 && contentSelectEndIndex !== -1) {
                contentPrice = +(userBasket.options.contentSelect.slice(contentSelectStartIndex + 2, contentSelectEndIndex));
            }

            const totalPrice = (price + contentPrice) * userBasket.count + packagingPrice * userBasket.count;
            return totalPrice;
        }

        userBasket.totalPrice = calculateTotalPrice(findItem);

        let cartItem = {}

        cartItem.photo = findItem.productSrc[0]
        cartItem.id = userBasket.productId
        cartItem.name = findItem.name
        cartItem.content = userBasket.options.contentSelect
        cartItem.package = userBasket.options.packagingSelect
        cartItem.count = userBasket.count
        cartItem.price = userBasket.totalPrice
    ...
    const onBuyEach = (id) => {
        ...
        let product = allProductData.find((item) =>
            item.id == findData.id
        )
